...
int getMin(int inArray[], int inStart, int inLast, int inNumThread)
{
    ...
    if (inLast > inStart)
    {
        ...
        {            //  split array in half & start a thread for each
            ...
            #pragma omp task
            min1 = getMin(inArray,inStart,half,(inNumThread/2));

            #pragma omp task
            min2 = getMin(inArray,(half + 1),inLast,(inNumThread/2));

            # pragma omp taskwait
        } else
        {       //   down to one thread - just do recursive getMin in this thread
